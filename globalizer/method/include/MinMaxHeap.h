...
//  File:      minmaxheap.h                                                //
//                                                                         //
//  Purpose:   Header file for minmaxheap class                            //
//                                                                         //
//  Author(s): ???                                                         //
//                                                                         //
/////////////////////////////////////////////////////////////////////////////

#ifndef __MINMAXHEAP_H__
#define __MINMAXHEAP_H__
...
class MinMaxHeap
{
  ...
  int trickleUp_(unsigned int index) // вспомогательный метод для всплытия
  {
    ...
    {
      QueueBaseData* i1 = static_cast<QueueBaseData*>(m_heap[index].pValue);
      QueueBaseData* i2 = static_cast<QueueBaseData*>(m_heap[index_grandparent].pValue);

      swapLinkedElems(m_heap[index_grandparent], m_heap[index]);
      std::swap(m_heap[index_grandparent], m_heap[index]);

      i1->SetQueueElementa(&m_heap[index_grandparent]);
      i2->SetQueueElementa(&m_heap[index]);
  ...
  int trickleUp(unsigned int index) // размещаем узел на соответствующем уровне (Min или Max)
  {
    ...
    if (isOnMinLevel(index))
    {
      ...
      {
        QueueBaseData* i1 = static_cast<QueueBaseData*>(m_heap[index_parent].pValue);
        QueueBaseData* i2 = static_cast<QueueBaseData*>(m_heap[index].pValue);

        swapLinkedElems(m_heap[index_parent], m_heap[index]);
        std::swap(m_heap[index_parent], m_heap[index]);

        i1->SetQueueElementa(&m_heap[index]);
        i2->SetQueueElementa(&m_heap[index_parent]);
  ...
  void trickleDown_(unsigned int index) // вспомогательный метод для погружения
  {
    ...
    QueueBaseData* i1 = static_cast<QueueBaseData*>(m_heap[index].pValue);
    QueueBaseData* i2 = static_cast<QueueBaseData*>(m_heap[smallestNode].pValue);
    ...
    i1->SetQueueElementa(&m_heap[smallestNode]);
    i2->SetQueueElementa(&m_heap[index]);
    ...
    if (smallestNode - left > 1)
    {
      ...
      if (m_compare(m_heap[parent(smallestNode)], m_heap[smallestNode]) ^ MaxLevel)
      {
        ...
        int par0 = parent(smallestNode);

        i1 = static_cast<QueueBaseData*>(m_heap[par0].pValue);
        i2 = static_cast<QueueBaseData*>(m_heap[smallestNode].pValue);

        swapLinkedElems(m_heap[parent(smallestNode)], m_heap[smallestNode]);
        par0 = parent(smallestNode);
        std::swap(m_heap[parent(smallestNode)], m_heap[smallestNode]);

        i1->SetQueueElementa(&m_heap[smallestNode]);
        i2->SetQueueElementa(&m_heap[par0]);
      }
  ...
  void deleteElement(unsigned int index) // удаление элемента из кучи
  {
    ...
    //TBaseInterval* i1 = (TBaseInterval*)m_heap[index].pValue;
    //TBaseInterval* i2 = (TBaseInterval*)m_heap[m_currentheapsize - 1].pValue;

    QueueBaseData* i1 = static_cast<QueueBaseData*>(m_heap[index].pValue);
    QueueBaseData* i2 = static_cast<QueueBaseData*>(m_heap[m_currentheapsize - 1].pValue);
    ...
    i1->SetQueueElementa(&m_heap[m_currentheapsize - 1]);
    i2->SetQueueElementa(&m_heap[index]);
...
public:
  ...
  T & findMax() const // возвращает максимум за O(1) без удаления из кучи
  {
